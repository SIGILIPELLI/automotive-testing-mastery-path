...
## How It Actually Works: bit timing and the sample point

Every node on a CAN bus generates its own clock — there is no shared
clock line. What keeps 500 kbit/s nodes from drifting apart mid-frame is
the **bit timing** state machine, and understanding it is what lets you
diagnose "works most of the time, occasionally corrupts a frame"
problems that look electrical but are actually configuration.

Each bit period is divided by the CAN controller into a fixed number of
**time quanta (TQ)**, split into four segments:
...
- **Sync_Seg** — every node resynchronizes here on an edge (recessive→
  dominant transition).
- **Prop_Seg** — absorbs physical propagation delay across the bus and
  through the transceivers, so a bit generated by a node at one end of
  the harness is guaranteed to have physically arrived everywhere before
  anyone samples it.
- **Phase_Seg1 / Phase_Seg2** — the node samples the bus at the boundary
  between them (the **sample point**, conventionally 75-87.5% into the
  bit). Every edge that arrives slightly early or late nudges Phase_Seg1
  or Phase_Seg2 by up to the **Synchronization Jump Width (SJW)**,
  continuously re-aligning every node's local oscillator without a
  shared clock — this is why CAN doesn't need one.

Two nodes with the *same nominal bit rate* but different sample-point
percentages (a common CANoe channel misconfiguration, or a mismatch
between the DUT's bootloader-configured timing and its application
timing) will each individually transmit clean-looking frames, but their
notion of "where the middle of a bit is" disagrees — under load, with
real oscillator tolerance and stub-length propagation delay stacking up,
this shows up as sporadic bit errors that get blamed on wiring. When you
configure a CANoe channel's bit rate (Module 4), you are really setting
this whole TQ/sample-point structure, not just a baud number — and a
tester who only matches the bit rate while leaving a different
sample-point percentage than the DUT can reproduce exactly the
"intermittent, load-dependent" CRC error pattern described below without
any physical fault at all.

...
**Why the seed can't just be a fixed value, and what "pseudo-random"
actually has to mean here.** If `DiagSecurityAccessRequestSeed`
returned the same seed every time (or a seed following a predictable
sequence, like an incrementing counter), an attacker could compute the
key once — by any means, including a single successful legitimate
unlock captured off the bus — and replay that exact key indefinitely
without ever needing to know the underlying key-derivation algorithm.
A correctly implemented UDS `0x27` seed generator uses a
cryptographically adequate random or pseudo-random source (commonly
seeded from a hardware RNG or a counter mixed with an unpredictable
runtime value) specifically so that observing N successful seed/key
exchanges gives an attacker no useful information for predicting the
N+1th seed. `tc_SecurityAccess_CorrectKeyUnlocks` as written only
proves the happy path works with one valid seed/key pair — it does
not, by itself, prove seed unpredictability, which is why a real
security test plan adds a dedicated statistical or repeated-sampling
check on seed values (comparing many captured seeds for detectable
patterns) as a separate testcase from the functional unlock check.

**Why the lockout has to be measured in ECU-persistent time, not
session time.** A naive lockout implementation might track failed
attempts in RAM and reset the counter on any ECU reset or power cycle
— which defeats the mechanism against exactly the attacker model it
exists for, since an attacker with physical or network access to
power-cycle the ECU (or trigger a reset via an unrelated diagnostic
service) gets unlimited retry attempts, just three at a time. A
correct implementation persists the failed-attempt count and the
lockout-expiry timestamp in non-volatile memory (EEPROM/flash) or
derives the delay from a monotonic counter unaffected by reset, so the
required time delay (UDS NRC `0x37`, "requiredTimeDelayNotExpired")
continues counting across a reset. This is exactly the mechanism
Exercise 1 is pointing at — the lockout test is incomplete without
a reset-survival check, because a lockout that survives inspection but
not a power cycle looks identical to a correct one in the base test.

**Why TARA's attack-path column is the thing that actually drives
which ECU gets deep security testing.** A naive risk model might rate
an ECU's cybersecurity test depth purely by what that ECU itself
controls (a braking ECU gets deep testing, an infotainment unit
doesn't). But ISO 21434's attack-path concept means an asset's actual
exposure depends on everything reachable *before* it in the network
topology — the TARA example's attack path (infotainment → gateway →
braking ECU) means the infotainment unit's own security testing
depth should be driven partly by what it can reach downstream, not
just by its own low intrinsic asset value, because a gateway that
fails to enforce message-origin segmentation turns a low-value,
externally-reachable ECU into the practical entry point for
compromising a high-CAL one. This is the mechanical reason gateway
segmentation testing (confirming the gateway actually drops or
rejects out-of-profile messages from the infotainment side rather
